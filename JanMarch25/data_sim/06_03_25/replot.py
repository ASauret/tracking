...
path = '/machfs/sauret/SharedCodes/tracking/data_sim/06_03_25/'
...
plt.tight_layout()
plt.savefig(path+'data_EBS_full_energy.png', dpi=100)
plt.show()

# plt.plot(x, ie[y==0].T, label='b = 6.05m')
# plt.plot(x2, ie2[y2==0].T, label='b = 6.05m - low emit')
plt.plot(x3, ie3[y3==0].T, label='b = 6.05m - full energy linac')
plt.xlabel(r'x [m]')
plt.ylabel(r'I.E. [%]')
plt.legend()
plt.tight_layout()
plt.savefig(path+'data_EBS_full_energy_cut.png',dpi=100)
plt.show()
