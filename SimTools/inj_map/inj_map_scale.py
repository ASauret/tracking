...
for i in range(len(f)):
    ...
    nlk_at = kicker.gen_at_elem('NLK', length, pn/brho)
    drift = at.Drift('DR_NLK', 0.0941)
    nlks = at.Lattice([nlk_at, drift, nlk_at, drift, nlk_at], energy=6.0e9, periodicity=1)
    ...
    for j in range(len(beta)):
        ...
        print('Beam tracked')
print('Matrix size :', len(f)*len(beta))
...
ie = ie.reshape((len(f), len(beta))).T

pickle.dump({'f':f, 'b':beta, 'ie':ie}, open('/machfs/sauret/injection/results/scale_beta_scan2.pkl', 'wb'))
