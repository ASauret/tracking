...
for i in range(len(x)):
    ...
    for j in range(len(xp)):
        print(j)
        dx6 = np.array([injoff+x[i], -xp0 +xp[j], 0.0, 0.0, 0.0, 0.0]) 
        sigma_mat = at.sigma_matrix(betax=5.0, betay=l0.beta[1],
...
runner = runners.Injeff_runner_EBS_thin(all_beams_errors, '/machfs/sauret/injection/lattices/before_injury/injection_nlk_mb.mat', nturns)
runner.slurm.clean()
...
pickle.dump({'x':x, 'xp':xp, 'ie':ie}, open('/machfs/sauret/injection/results/full_linac/hor_ps_scan.pkl', 'wb'))
